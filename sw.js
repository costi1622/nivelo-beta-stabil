...
self.addEventListener('install', e => {
  ...
  e.waitUntil(
    caches.open(CACHE)
      // `cache: 'reload'` ocolește cache-ul HTTP al browserului. Fără el, la instalarea
      // unei versiuni noi fișierele se pot lua din copia veche pe care browserul o mai
      // ține (GitHub Pages trimite un max-age de câteva minute), iar service worker-ul
      // NOU ajunge să pună în cache-ul NOU exact pagina veche. De acolo, „Reîncarcă"
      // părea că nu face nimic: ștergeai cache-ul vechi ca să primești tot vechiul.
